...
// ------------------------------------------ BITONIC SORTS ------------------------------------------ //
// W_CMPXCHG
void cmpxchg_DOUBLE(__global volatile double* A, __global volatile double* B, bool dir)
{
	if ((!dir && *A > *B) || (dir && *A < *B))
	{
		double t = *A;
...
// W_BITONIC_MERGE
void bitonic_merge_DOUBLE(int id, __global volatile double* data, int N, bool dir)
{
	for (int i = N/2; i > 0; i /= 2)
	{
		if ((id % (i * 2)) < i)
...
// W_BitonicSort
__kernel void sort_bitonic_DOUBLE(__global volatile double* data)
{
	int id = get_global_id(0);
	int N = get_global_size(0);
...
__kernel void bitonic_local_DOUBLE(__global volatile const double* in, __global volatile double* out, __local volatile double* cache)
{
	int lid = get_local_id(0);
	int wg = get_local_size(0);
...
// ------------------------------------------ TWO-STAGE_REDUCTION ------------------------------------------ //
// REDUCE_MIN_LOCAL
__kernel void reduce_min_DOUBLE(__global volatile const double* in, __global volatile double* out, __local volatile double* scratch)
{
	int id = get_global_id(0);
	int lid = get_local_id(0);
...
// REDUCE_MIN_GLOBAL
__kernel void reduce_min_global_DOUBLE(__global volatile const double* in, __global volatile double* out)
{
	int id = get_global_id(0);
	int lid = get_local_id(0);
...
// REDUCE_MAX_LOCAL
__kernel void reduce_max_DOUBLE(__global const volatile double* in, __global volatile double* out, __local volatile double* scratch)
{
	int id = get_global_id(0);
	int lid = get_local_id(0);
...
// REDUCE_MAX_GLOBAL
__kernel void reduce_max_global_DOUBLE(__global volatile const double* in, __global volatile double* out)
{
	int id = get_global_id(0);
	int lid = get_local_id(0);
...
// ------------------------------------------ VARIANCE ------------------------------------------ //
__kernel void sum_sqr_diff_DOUBLE(__global volatile const double* in, __global volatile double* out, __local volatile double* scratch, double mean)
{
	int id = get_global_id(0);
	int lid = get_local_id(0);
